...
var GameState = {
  ...
  create: function() {
    ...
    animalData.forEach(function(item) {
      animal = self.animals.create(-1000, self.game.world.centerY, item.key, 0);

      animal.customParams = { text: item.text, sound: self.game.add.audio(item.audio) };
  ...
  switchAnimal(sprite, event) {
    ...
    newAnimalMoviment.onComplete.add(function() {
      self.isMoving = false;
      self.showText(newAnimal);
    });

    newAnimalMoviment.start();

    var currenteAnimalMoviment = this.game.add.tween(this.currentAnimal);
    currenteAnimalMoviment.to({x: endX}, 1000);
    currenteAnimalMoviment.start();
